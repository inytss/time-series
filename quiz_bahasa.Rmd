...
Sebelum kita membuat model peramalan, mari kita amati data kita. Apakah data yang kita miliki sudah berupa objek *time series*? Apabila belum, silahkan buat objek *time series* berdasarkan data `theft` menggunakan fungsi `ts()` dengan frekuensi yang digunakan adalah 365 dan simpanlah ke dalam objek `theft_ts`. Untuk mengetahui karakteristik dari objek *time series* `theft_ts`, ambilah data pada 15 tahun pertama dengan menggunakan fungsi `head()` dan buatlah visualisasinya menggunakan fungsi `autoplot()` dari *package* `forecast`. Jawablah pertanyaan pertama berdasarkan plot yang telah dibuat. 

*Note: Ketika Anda mengambil data 15 tahun pertama untuk visualisasi, Anda tidak perlu menyimpannya kedalam suatu objek dan dapat langsung memvisualisasikannya.*
...
___
1. Manakah pernyataan di bawah ini yang **BENAR** terkait plot *time series* tersebut?
  - [ ] Objek adalah *additive time series* karena seiring dengan meningkatnya tren, amplitudo/besaran dari aktivitas *seasonal* juga meningkat.
  - [ ] Objek adalah *multiplicative time series* karena seiring dengan meningkatnya tren, amplitudo/besaran dari aktivitas *seasonal* cenderung konstan.
  - [ ] *Time series* tidak memiliki pola tren, *seasonal*, dan *error*
  - [ ] *Time series* memiliki pola tren, *seasonal*, dan *error*
...
6. Berdasarkan hasil di atas, manakah dari pernyataan berikut yang **BENAR**?
  ...
  - [ ] Perbedaan nilai *mean absolute percentage error* antara model ARIMA dan *Holt-Winters* adalah 0.53%
