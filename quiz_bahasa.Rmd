...
1. Manakah pernyataan dibawah ini yang **BENAR** mengenai *time series*?
  ...
  - [ ] Multiplikatif *time series* adalah multiplikatif karena ketika trennya meningkat, amplitudo/besaran dari aktifitas *seasonal*-nya meningkat
...
Mari kita mengekplorasi model lain untuk meramalkan data `train_theft` menggunakan algoritme ARIMA. Silahkan bangunlah sebuah model ARIMA menggunakan fungsi `stlm()`, aturlah pada argumen `method` menjadi "arima" dan jangan lupa untuk mengatur argumen `s.window` menjadi sejumlah frekuensi data time series yang dimiliki lalu simpanlah kedalam objek `model_arima`.
...
6. Berdasarkan hasil diatas, manakah dari pernyataan berikut yang **BENAR**?
  ...
  - Perbedaan nilai *mean absolute percentage error* antara model ARIMA dan *Holt-Winters* sekitar 0.53%
___
